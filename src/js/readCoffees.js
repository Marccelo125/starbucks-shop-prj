...
export async function readCoffees(data) {
  data.forEach((coffee) => {
    ...
    <img src='${
      coffee.img
    }' class="card-img-top img-fluid" alt="Imagem do café ${coffee.id}">
    <div class="card-body">
      <h5 class="card-title text-center" id="coffe-title">${coffee.id} - R$${
      coffee.features.price
    }</h5>
...
requestApi("./src/json/shopData.json")
  .then((data) => {
    readCoffees(data);
  })
  .catch((error) => {
